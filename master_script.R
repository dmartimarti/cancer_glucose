...
jointresults.multi %>%
  ...
  geom_point(aes(colour = z_logFC), alpha = 0.8, size = 2)+
  scale_size(range = c(1,5), name = expression(paste(italic('C. elegans'), '\nphenotype', sep = ''))) +
  scale_colour_gradientn(colours = gradcolours,
                         breaks = c(0,1,2,3,4), limits = c(0,4), guide = "legend", name = expression(atop(italic('C. elegans'),' phenotype'), sep = "")) + # I use 'atop' to force a new line
  scale_y_continuous(breaks = -5:5) +
  coord_cartesian(xlim = c(-4, 3), ylim = c(-3, 4)) + # coordinates for BW
